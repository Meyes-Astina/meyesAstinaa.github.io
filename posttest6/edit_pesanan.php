...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($gambar)) {
        ...
        $result = mysqli_query($koneksi, "SELECT gambar FROM tb_art WHERE id=$id");
        $row = mysqli_fetch_assoc($result);
        $gambar_lama = $row['gambar'];
        if (!empty($gambar_lama)) {
            unlink($gambar_path . $gambar_lama);
        }

        // Unggah gambar baru
        move_uploaded_file($gambar_tmp, $gambar_path . $gambar);

        // Update data pesanan termasuk gambar
        $sql = "UPDATE tb_art SET jenisseni='$jenisseni', jenispesanan='$jenispesanan', deskripsi='$deskripsi',
    gambar='$gambar', nama='$nama', age='$age', gmail='$gmail', katasandi='$katasandi' WHERE id=$id";
    } else {
        // Jika tidak ada gambar baru dipilih, update data tanpa mengubah gambar
        $sql = "UPDATE tb_art SET jenisseni='$jenisseni', jenispesanan='$jenispesanan', deskripsi='$deskripsi',
        ...
        <section class="order-form">
            ...
            <form method="POST">
                ...
                <div class="input-group">
                    ...
                    <input type="password" id="katasandi" name="katasandi" value="<?php echo $row['katasandi']; ?>" required>
                    <div class="input-group">
                        <label for="jenisseni">Jenis Seni:</label>
                        <select id="jenisseni" name="jenisseni" required>
                            <option value="Anime Style" <?php echo ($row['jenisseni'] === 'Anime Style') ? 'selected' : ''; ?>>Anime Style
                            </option>
                            <option value="Game Style" <?php echo ($row['jenisseni'] === 'Game Style') ? 'selected' : ''; ?>>Game Style
                            </option>
                            <option value="Semi Realistic" <?php echo ($row['jenisseni'] === 'Semi Realistic') ? 'selected' : ''; ?>>Semi Realistic
                            </option>
                        </select>
                        <label for="jenispesanan">Jenis Pesanan:</label>
                        <select id="jenispesanan" name="jenispesanan" required>
                            <option value="Bust Up" <?php echo ($row['jenispesanan'] === 'Bust Up') ? 'selected' : ''; ?>>Bust Up</option>
                            <option value="Half Body" <?php echo ($row['jenispesanan'] === 'Half Body') ? 'selected' : ''; ?>>Half Body
                            </option>
                            <option value="Full Body" <?php echo ($row['jenispesanan'] === 'Full Body') ? 'selected' : ''; ?>>Full Body
                            </option>
                        </select>
                    </div>
                    ...
                    <label for="gambar">Contoh Gambar Pesanan (PNG, JPEG, atau GIF):</label>
